        ...
        while idx < len(lines):
            ...
            try:
                yield {
                    ...
                    "atom_name": atom_name,
                    "alt_id": clean_cif_missing_value(row.get("label_alt_id", "")),
                    "element": first_present(row, "type_symbol"),
                    "occupancy": float(first_present(row, "occupancy") or 1.0),
                    "b_factor": float(first_present(row, "B_iso_or_equiv") or 0.0),
                    "x": float(row["Cartn_x"]),
                    "y": float(row["Cartn_y"]),
                    "z": float(row["Cartn_z"]),
                }
